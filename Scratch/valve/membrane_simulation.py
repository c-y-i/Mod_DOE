...
_MU_elastic = 31_700 
_JM_elastic = 39.6
# material constants - TODO stiff solved from uniaxial tensile testing.
_MU_stiff = _MU_elastic
_JM_stiff = _JM_elastic
...
#################### UPDATED FUNCTIONS ####################
# __R0__ = 1e-9 # assume very small r0 (no contact area) - NOTE can add as input if we want
# __Rf__ = 3.75e-3 # outer radius of dimple
__R0__ = 0.0254
__Rf__ = 0.075

# simplified 'obtain_qoi_plane' function for single piece / valve - Primary simulation function for pressure-height plane generation
def obtain_ph_plane(h, material_type,
    ...
    Args:
        ...
        material_type (string): sequence of material types
    ...
    best_x, PP = obtain_valve_x(h, material_type,
    ...
    heights = vmap(simulate_valve_height, in_axes=(0, 0, None, None))(best_x, PP, h, material_type)
    ...
    if return_best_x:
        return PP, heights, best_x
    else:
        return PP, heights

# simplified 'obtain_qoi_x' function for single piece / valve 
def obtain_valve_x(h, material_type, p_min=50, p_max=8_000, num_ps=100,
            init_guess=5, dx_init=0.01, atol=1e-4, max_iter=500,
            verbose=True):
    '''
    Args:
        h (float): thickness of membrane
        material_type (string): sequence of material type
        p_min (float): minimum pressure being applied to the membrane
        p_max (float): maximum pressure being applied to the membrane
        num_ps (int): resolution of pressure grid
        init_guess (float): initial guess for shooting x
    ...
    PP = jnp.linspace(p_min, p_max, num_ps) # shape (num_ps,)

    # initialize values
    dx = jnp.ones_like(PP)*dx_init
    current_x = jnp.ones_like(dx) * init_guess

    # compute values
    best_x = jnp.ones_like(dx) * init_guess # best x value so far
    best_val =vmap(shoot_valve_x, in_axes=(0, 0, None, None))(best_x, PP, h, material_type) # best value so far
    ...
    for i in trange(max_iter):
        ...
        current_val = vmap(shoot_valve_x, in_axes=(0, 0, None, None))(current_x, PP, h, material_type)
    ...
    if verbose:
        ...
        if good_vals.sum()<len(good_vals):
            print(f'Could not find roots for the remaining {num_ps - good_vals.sum()}.')
    return best_x, PP

# simplified version of 'shoot_x' function for single piece / valve
def shoot_valve_x(x, p, h, material_type):
    full_sols = simulate_valve(x, p, h, material_type)
    return full_sols[-1].ys[-1,1] - 1


# simplified version of #simulate_ringed_membrane - for single piece / valve
def simulate_valve(x, p, h, material_type):
    '''
    Args:
        ...
        h (float): thickness of membrane
    '''
    # set material constants - NOTE we can add more materials as an input later
    # mat_constants = (_MU_elastic, _JM_elastic, h)

    if material_type == 'elast':
        mat_constants = (_MU_elastic, _JM_elastic, h) # perhaps Ecoflex 00-30?
    elif material_type == 'stiff':
        mat_constants = (_MU_stiff, _JM_stiff, h) # perhaps DragonSkin 20?

    # compute initial conditions
    # R0 = change_material[0] # get contact area
    R0 = __R0__ # assume very small r0 (no contact area) - NOTE can add as input if we want
    Rf = __Rf__ # outer radius of dimple
    W, W_r, W_theta, W_rr, W_r_theta = W_grads(x, 1., mat_constants)
    # beta_0 = jnp.arccos( (-F+jnp.pi * p * R0**2) / (2*jnp.pi*h*R0*W_r) )
    beta_0 = 0 # assume for very small r0, center is flat
    y0 = jnp.array([x, 1., beta_0])

    r_init = R0 # where piece starts
    r_end = Rf

    sol = solve_valve_ode(y0, p, r_init, r_end, mat_constants)

    return [sol]

# simplified simulate_height function for single piece / valve
def simulate_valve_height(x,p, h, material_type):
    sol = simulate_valve(x, p, h, material_type)[0]
...
# update of 'vector_field' - this determines the evolution of the ODE system
def valve_vector_field(r, y, args):
    # unpack y
...
_valve_ode_term = ODETerm(valve_vector_field)
_ode_solver = Tsit5()
_ode_dr0 = 0.000075

def solve_valve_ode(y0, p, r_init, r_end, mat_constants):
    # solve ode
    args = mat_constants, p
    saveat = SaveAt(ts=jnp.linspace(r_init, r_end, 1000))
    sol = diffeqsolve(_valve_ode_term, _ode_solver, r_init, r_end, _ode_dr0, y0, args=args, saveat=saveat)
    return sol
...
def simulate_ringed_membrane(x, F, p, h, change_material, material_type):
    ...
    # compute initial conditions
    # R0 = change_material[0] # get contact area
    R0 = 1e-12 # assume very small r0 (no contact area)
    W, W_r, W_theta, W_rr, W_r_theta = W_grads(x, 1., mat_constants)
    # beta_0 = jnp.arccos( (-F+jnp.pi * p * R0**2) / (2*jnp.pi*h*R0*W_r) )
    beta_0 = 0 # assume for very small r0, center is flat
    y0 = jnp.array([x, 1., beta_0])
